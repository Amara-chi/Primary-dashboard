      ...
      >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
          {achievements.map((achievement, index) => (
            <motion.div
              key={index}
              className={`bg-white dark:bg-slate-700 rounded-xl p-4 lg:p-6 shadow-lg text-center border-2 ${getRarityColor(achievement.rarity)}`}
            ...
            >
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <motion.span 
                  className={`px-2 py-1 rounded-full text-xs font-bold text-white bg-gradient-to-r ${getRarityColor(achievement.rarity)}`}
                  animate={{ scale: [1, 1.1, 1] }}
                  transition={{ duration: 2, repeat: Infinity }}
                >
